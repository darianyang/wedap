...
def main():
    #kin_path = ["2kod_v02/kinetics/C2_M2_1/direct.h5"]
    #kin_path = ["1a43_v02/kinetics/C2_M2_1/direct.h5"]
    kin_path = ["1a43_v02/kinetics/C2_M2M1-Dist_1/direct.h5"]
   
    fig, ax1 = pyplot.subplots(figsize=(7.25, 8))

    ca = CorrelationAnalysis(kin_path, 1, fi=1, li=None)
    ca.plot(xlims=(0,200), figname='autocorrel_comparison.png', ax=ax1, format_axis=True)

    fig.subplots_adjust(bottom=0.2, hspace=0.0)

    ax1.legend(frameon=False)
    #ax1.set_xlabel('lag time')
    #ax1.text(0.05, 0.6, 'autocorrelation of weight/number flux into target state', 
    #         transform=fig.transFigure, ha='center', va='center', rotation='90')
    #ax1.set_xticklabels(['' for i in range(4)])
    #ax1.set_yticklabels(['-1.0', '-0.5', '0.0', '0.5', ''])
    #ax1.text(1.05, 0.5, u"holo N':apo N\nN\u2192N'", transform=ax1.transAxes, 
    #         rotation='90', ha='center', va='center')

    #pyplot.savefig('autocorrel_comparison.png')
    pyplot.show()
